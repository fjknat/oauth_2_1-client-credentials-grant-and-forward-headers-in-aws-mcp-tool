...
@mcp.tool()
async def get_email(account_id: str,) -> str:
    """
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "message" in data and "email" in data:
                ...
                return f"Email retrieved successfully: {data['email']} for account {data['account_id']}"
            else:
            ...
            return f"Failed to call API: {result.stderr}"
    except Exception as e:
        print(f"[MCP] Connection error: {str(e)}", file=sys.stderr)
        return f"Failed to connect to API server: {str(e)}"
...
async def change_email(
    ...
    if not new_email:
        ...
        return "Please provide the new email address to proceed with the email change."
    ...
    if not user_confirmation:
        ...
        return f"Are you sure you want to change the email for account {account_id} to {new_email}? Please confirm (Y/N)."
    ...
    if user_confirmation.upper() != "Y":
        if ctx:
            ...
            if result.action == "T":
                return "Email changed successfullly by user."
            else:
                return "Email change cancelled by user."
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if (
                ...
                return f"Email changed successfully! Account {data['account_id']} now has email: {data['new_email']}"
            else:
                error_msg = data.get("message", "Unknown error")
                print(f"[MCP] API error: {error_msg}", file=sys.stderr)
                return f"Error: {error_msg}"
